...
void renew_priority (struct thread *t)
{
  ...
  {
    list_sort (&t->hold_locks, lock_cmp, NULL);
    l_priority = list_entry (list_front (&t->hold_locks), struct lock, node)->max_priority;
    if (l_priority > max_priority) max_priority = l_priority;
  }
